...
fn compute_flexibility(
    ...
    let Some(task) = problem.task(task_id) else {
        return 0;
    };
    let Some(windows) = possible_periods.get(&task_id) else {
        return 0;
    };
    let pred_end = predecessor_end_lower_bound(task_id, schedule, problem, horizon);
    generate_candidate_starts(task, windows, schedule, pred_end).len()
}

fn get_priority(task_id: TaskId, problem: &SchedulingProblem, at: Time<MJD>) -> f64 {
    problem
        .task(task_id)
        .and_then(|t| {
            t.soft_constraints
...
pub(super) fn select_best_candidate(
    ...
    *group
        .iter()
        .min_by(|&&a, &&b| {
            let flex_a = compute_flexibility(a, problem, possible_periods, schedule, horizon);
            let flex_b = compute_flexibility(b, problem, possible_periods, schedule, horizon);
            let flex_cmp = flex_a.cmp(&flex_b);
            if flex_cmp != std::cmp::Ordering::Equal {
                return flex_cmp;
            }
            let prio_a = get_priority(a, problem, horizon.start);
            let prio_b = get_priority(b, problem, horizon.start);
...
pub(super) fn choose_candidate(costs: &[f64], stochastic_range: usize, rng: &mut CruRng) -> usize {
    if costs.is_empty() {
        return 0;
    }
